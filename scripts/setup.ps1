...
$dbReady = $false
while ($retries -gt 0) {
    $status = (docker inspect --format='{{.State.Health.Status}}' ppk_db 2>$null)
    if ($status -eq "healthy") {
        $dbReady = $true
        break
    }

    docker compose exec -T db sh -c 'MYSQL_PWD="$MYSQL_ROOT_PASSWORD" mysqladmin ping 2>/dev/null' | Out-Null
