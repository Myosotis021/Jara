...
<head>
    ...
    <title>@yield('title', 'JARA - Manajemen Workspace')</title>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between h-16">
                <div class="flex items-center space-x-6">
                    ...
                    <a href="{{ route('workspaces.index') }}" class="text-sm font-semibold text-gray-700 hover:text-blue-600 px-3 py-2 rounded-md transition">
                        Workspaces
                    </a>
                </div>
                    ...
                    @auth
                        ...
                        @if (Route::has('logout'))
                            <form action="{{ route('logout') }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" class="text-sm font-medium text-red-600 hover:text-red-800 px-3 py-1.5 rounded-lg border border-red-200 hover:bg-red-50 transition">
                                    Logout
                                </button>
                            </form>
                        @endif
